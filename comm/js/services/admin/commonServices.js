...
angular.module('common.services', ['LocalStorageModule'])
	.factory('common', function ($http, $location, $route, $state, $window, $timeout, $q, $templateCache, $translate, cache, cookies, CONSTANTS, SITEMAP, $mdDialog) {
        ...
        common.setPortalOrgKey = function (portalOrgId) {
            cache.setPortalOrgKey(portalOrgId);
        };

        common.clearPortalOrgKey = function () {
            cache.clearPortalOrgKey();
        };
				...
				for (var i=0; i<leftMenus.length; i++) {
					...
					if (!menuItem.menuDisplayNo && !menuItem.subPage) {
						...
						if (isMenuView) {
							...
							if (menuItem.ngClick) {
                                templateHtml	= '<a ng-click="' + menuItem.ngClick + '">';
                            } else {
                                if (menuItem.menuNoLink) {
                                    templateHtml	= '<a href="">';
                                } else {
                                    if (menuItem.defaultUrl) {
                                        templateHtml = '<a href="#' + menuItem.url + '">';
                                    } else {
                                        templateHtml = '<a href="">';
                                    }
                                }
                            ...
                            if (!menuItem.subMenu) {
                                templateHtml += '<span class="glyphicon glyphicon-menu-left"></span>';
                            }
                            templateHtml += '</a>';
                            var menuObj = { mainKey : siteMap.mainKey, key: menuItem.key, id: i, templateHtml : templateHtml, iconClassName : iconClassName, className :  className.trim() };
                            if (!menuItem.subMenu) {
                                leftMenuList.push(menuObj);
                                if (className != "active" && menuItem.open && leftMenus[i+1] && leftMenus[i+1].subMenu) {
                                    menuObj.className = "open";
                                }
                            } else {
		...
		common.getNavigationRootTree = function (sltSiteMap, navigationTree) {
            ...
            if (angular.isObject(sltSiteMap.parentSiteMap) && sltSiteMap.parentSiteMap.name != sltSiteMap.rootName) {
                ...
                if (sltSiteMap.icon) {
                    navigationTree += '<li class="' + sltSiteMap.icon + '">';
                } else {
                    navigationTree += '<li>';
                }
            }
            ...
            promise.error = function (fn) {
                promise.then(null, function (response) {
                    ...
                    if (response.status == 307) {
                        if (!common.isMassageShowAlert) {
                            common.isMassageShowAlert	= true;
                            common.showAlertError($translate.instant("label.login"), $translate.instant("message." + response.data.message)).then(function () {
                                common.isMassageShowAlert	= false;
                            ...
                            try {
	                            if (response.data && response.data.message) {
		                            if (response.data.status == 403 && response.data.message == "OAuth2 access denied.") {
			                            ...
			                            common.isMassageShowAlert = true;
			                            common.showAlertWarningHtml(errTitle, errMessage).then(function () {
				                            common.isMassageShowAlert = false;
			                            });
		                            }
                                } else {
                                    ...
                                    common.isMassageShowAlert = true;
                                    common.showAlertWarningHtml($translate.instant("label.error"), response.statusText).then(function () {
                                        common.isMassageShowAlert = false;
                                    });
                                }
                            } catch (e) {
        ...
        // promise 받기 success, error, status 307을 제외한 에러 자동 open 제외. 공통에서 사용
        common.retrieveCommResource = function (promise, finallyFn) {
            promise.success = function (fn) {
                promise.then(function (response) {
                    if(angular.isUndefined(response)) response = {data:null,status:null,headers:null};
                    var newToken = response.headers(_CSRF_TOKEN_HEADER_NAME_);
                    ...
                    try {
                        ...
                        if (response.status == 307) {
                            if (!common.isMassageShowAlert) {
                                ...
                                $timeout(function () {
                                    common.showAlertError($translate.instant("label.login"), $translate.instant("message." + message)).then(function () {
                                        common.isMassageShowAlert = false;
                            ...
                            if (!common.isMassageShowAlert) {
                                if (response.data && response.data.message) {
                                    ...
                                    $timeout(function () {
                                        common.showAlertWarningHtml($translate.instant("label.error"), message).then(function () {
                                            common.isMassageShowAlert = false;
                                        });
                                    }, 1000);
                                }
        ...
        common.moveCommHomePage = function () {
            if (CONSTANTS.commHomeUrl.substring(0, 4) == "http" || CONSTANTS.commHomeUrl.indexOf(".html") > -1 || CONSTANTS.commHomeUrl.indexOf("#") > -1) {
                common.locationHref(CONSTANTS.commHomeUrl);
            } else {
                common.locationPath(CONSTANTS.commHomeUrl);
            }
        };
        ...
        // alert
        common.showAlertError = function (title, textContent) {
            return common.showAlert(title, textContent, "error");
        };

        // alert
        common.showAlertWarning = function (title, textContent) {
            return common.showAlert(title, textContent, "warning");
        };

		// alert
		common.showAlert = function (title, textContent, alertType) {
        ...
        // alert
        common.showAlertErrorHtml = function (title, textContent) {
            return common.showAlertHtml(title, textContent, "error");
        };

        // alert
        common.showAlertWarningHtml = function (title, textContent) {
            return common.showAlertHtml(title, textContent, "warning");
        };

        // alert
		common.showAlertHtml = function (title, htmlContent, alertType) {
            var rootScope = common.getMainCtrlScope();
            rootScope.main.loadingMain = false;
            rootScope.main.loadingMainBody = false;
		...
		common.showDialog = function ($scope, $event, dialogOptions) {
					...
					if (!dialogOptions.btnTemplate) {
						if (dialogOptions.buttons) {
                            ...
                            if (dialogOptions.okBtnHide != true) {
                                dialogOptions.btnTemplate += '<button ng-hide="actionBtnHied" class="btn btn-add" ng-click="popDialogOk()" ng-hide="error || success" ng-disabled="dialogOptions.authenticate">{{ dialogOptions.okName }}</button>\n';
							}
                            if (dialogOptions.cancelBtnHide != true) {
                                dialogOptions.btnTemplate += '<button ng-hide="actionBtnHied" class="btn" data-dismiss="modal" ng-click="popCancel()">{{ dialogOptions.closeName }}</button>\n';
                            }
                        }
 					}
				}
			...
			if (!dialogOptions.controller) {
                dialogOptions.controller = function () {
