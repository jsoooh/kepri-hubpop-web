...
angular.module('common.services')
    .factory('user', function (common, cache, cookies, CONSTANTS) {
        ...
        user.passwordSecureCheck = function(email, pw) {

            // 1차 규칙, 사용금지 패스워드 확인
            if (pw == 'kepco123456/' || pw == 'Kepco123456/' || pw == 'kepco123456!' || pw == 'Kepco123456!') {
                console.log("사용금지 패스워드를 사용하셨습니다. 다른 패스워드를 사용하여 주시기 바랍니다.");
                return;
            }
            ...
            if (!pattern1.test(pw) || !pattern2.test(pw) || !pattern3.test(pw) || pw.length < 8 || pw.length > 16){
                console.log("8~16자 영문, 숫자, 특수문자 조합으로 구성하여야 합니다.");
                return;
            } 
            ...
            for (var i = 0; i < pw.length; i++) {
                if (i + 2 < pw.length) {
                    console.log(i, pw.charCodeAt(i) + 1 == pw.charCodeAt(i + 1), pw.charCodeAt(i + 1) + 1 == pw.charCodeAt(i + 2));
                    if (pw.charCodeAt(i) + 1 == pw.charCodeAt(i + 1) && pw.charCodeAt(i + 1) + 1 == pw.charCodeAt(i + 2)) {
                        console.log("연속된 비밀번호는 3자 이상 사용할 수 없습니다.");
                        return;
                    }
                }
            }
            ...
            // 최근 사용한 비밀번호 재사용 확인
            return common.retrieveResource(common.resourcePromise(CONSTANTS.uaaContextUrl + '/users/recentPasswordCheck', 'GET', {email: email, pw: pw}));
        }
        return user;
