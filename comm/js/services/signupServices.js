...
angular.module('common.services')
    .factory('signupService', function (common, CONSTANTS) {
        ...
        signupService.signup = function(memberInfo) {
            return common.retrieveResource(common.resourcePromise(CONSTANTS.uaaContextUrl + '/users', 'POST', memberInfo));
        };

        /* 이메일 인증 */
        signupService.verifyUser = function(verifyCode) {
            return common.retrieveResource(common.resourcePromise(CONSTANTS.uaaContextUrl + '/users/verify', 'PUT', verifyCode, 'application/x-www-form-urlencoded'));
        };

        /* 이메일 인증 재발송 */
        signupService.resendVerifyEmail = function(email) {
            return common.retrieveResource(common.resourcePromise(CONSTANTS.uaaContextUrl + '/users/verify-link', 'POST', email, 'application/x-www-form-urlencoded'));
        };
        ...
        signupService.checkByCompany = function(companyId) {
            return common.retrieveResource(common.resourcePromise(CONSTANTS.uaaContextUrl + '/companies/' + companyId + "/checkCompany", 'GET'));
        };
        ...
        signupService.checkByInvitation = function(email) {
            return common.retrieveResource(common.resourcePromise(CONSTANTS.uaaContextUrl + '/orgs/' + email + '/invitation', 'GET'));
        };
        ...
        signupService.preChkResendVerifyEmail = function(email) {
            return common.retrieveResource(common.resourcePromise(CONSTANTS.uaaContextUrl + '/users/preCheckUserByEmail?email=' + email, 'GET'));
        };

        /* 파일업로드*/
        signupService.fileUpload = function(fieldName, uploadFile) {
            return common.retrieveResource(common.fileUploader(CONSTANTS.uaaContextUrl + '/uploadfile/' + fieldName, 'POST', uploadFile));
        };
